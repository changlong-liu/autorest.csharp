...
{
    internal partial class LowLevelBaseTestWriter: LowLevelClientWriter
    {
        public BuildContext<LowLevelOutputLibrary> context;
        public CodeWriter writer = new CodeWriter();
        public HashSet<string>  variableNames = new HashSet<string>();

        public LowLevelBaseTestWriter(CodeWriter writer, BuildContext<LowLevelOutputLibrary> context) : base()
        {
            this.writer = writer;
        ...
        public static ExampleGroup? FindExampleGroup(BuildContext<LowLevelOutputLibrary> context, RestClientMethod restOperation)
        {
        ...
        public static bool HasExample(BuildContext<LowLevelOutputLibrary> context, LowLevelClientMethod? clientOperation)
        {
            if (clientOperation is null)
            {
                ...
                var exampleGroup = LowLevelBaseTestWriter.FindExampleGroup(context, clientOperation.RequestMethod);
        ...
        public void WriteFrameworkTypeExampleValue(CodeWriter writer, CSharpType cst, ExampleValue exampleValue, string variableName)
        {
            ...
            {
                writer.Append($"new {cst}(${LowLevelBaseTestWriter.FormatResourceId(exampleValue.RawValue!.ToString()!):L})");
            }
        ...
        public List<string> WriteOperationParameters(IEnumerable<Parameter> methodParameters, IEnumerable<Parameter> testMethodParameters, ExampleModel exampleModel)
        {
            ...
            foreach (var passThruParameter in methodParameters)
            {
                ...
                foreach (ExampleParameter exampleParameter in exampleModel.AllParameter)
                {
                    ...
                    {
                        paramName = WriteExampleParameterDeclaration(writer, exampleParameter, passThruParameter);
                    }
                }
                if (paramName is null)
                {
                    ...
                    {
                        writer.Line($"{passThruParameter.Type} {paramName} = null; /* Can't find this parameter in example, please provide value here!*/");
                    }
                    else
                    {
                        writer.Line($"{passThruParameter.Type} {paramName} = null;");
                    }
                }
        ...
        public void WriteParameterValue(Parameter parameter, ExampleModel exampleModel)
        {
            bool found = false;
            foreach (ExampleParameter exampleParameter in exampleModel.AllParameter)
            {
                if (parameter.Name == exampleParameter.Parameter.CSharpName())
                {
                    WriteExampleValue(writer, parameter.Type, exampleParameter.ExampleValue, "");
                    found = true;
                }
            }
            if (!found)
            {
                writer.Line($"default /* Can't find parameter {parameter.Name} in example, please provide value here!*/");
            }
        }
    }
}
