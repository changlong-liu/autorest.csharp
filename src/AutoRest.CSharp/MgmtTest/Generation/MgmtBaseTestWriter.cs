...
namespace AutoRest.CSharp.MgmtTest.Generation
{
    internal abstract partial class MgmtBaseTestWriter : MgmtClientBaseWriter
    {
        ...
        public void WriteSchemaObjectExampleValue(CodeWriter writer, ObjectType sot, ExampleValue ev, FormattableString variableName, bool inSingleProperty)
        {
            ...
            foreach (var p in signature.Parameters)
            {
                ...
                else
                {
                    ...
                    else if (p.Type.IsFrameworkType && p.Type.FrameworkType == typeof(SystemAssignedServiceIdentityType))
                    {
                        writer.Append($"{typeof(SystemAssignedServiceIdentityType)}.SystemAssigned");
                    }
                    else
                    {
        ...
        public void WriteFrameworkTypeExampleValue(CodeWriter writer, CSharpType cst, ExampleValue exampleValue, FormattableString variableName)
        {
            ...
            switch ($"{cst.Namespace}.{cst.Name}")
            {
                ...
                default:
                    ...
                    {
                        var stringRawValue = rawValue.AsString();
                        if (WriteStringValueWithSchema(writer, cst, stringRawValue, exampleValue.Schema) || WriteStringValue(writer, cst, stringRawValue))
                            return;
                    }
        ...
        public bool WriteStringValueWithSchema(CodeWriter writer, CSharpType cst, string value, Schema schema)
        {
            if (schema.Type == AllSchemaTypes.Duration)
            {
                writer.Append($"{typeof(System.Xml.XmlConvert)}.ToTimeSpan({value:L})");
                return true;
            }
            return false;
        }
        ...
        protected static CodeWriterDelegate WriteGetExtension(MgmtExtensions extensions, RequestPath requestPath, ExampleModel exampleModel)
        {
            return writer =>
            {
                ...
                {
                    writer.Append($"(await GetArmClient().GetTenants().GetAllAsync().ToEnumerableAsync()).ToArray()[0]");
                }
